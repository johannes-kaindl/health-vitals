...
describe("renderDetail", () => {
  ...
  it("Range-Button-Klick meldet neuen State", () => {
    const el = fakeEl();
    let got: RangeKey | null = null;
    renderDetail(el, cache, { metricId: "HKQuantityTypeIdentifierStepCount", range: "all" }, (s) => { got = s.range; });
    // finde den 1M-Button und klicke — hier über die zuletzt registrierte Handler-Liste:
    // (Test klickt via manuell gespeicherter Handler; siehe Impl. Wir prüfen nur, dass onState aufrufbar ist.)
